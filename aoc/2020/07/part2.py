...
class BagNetwork:
    ...
    def must_contain_count_all(self, inner: BagType) -> int:
        """
        Return a count of how many bags the given bag type must contain.
        """
        # list of bag, quantity
        to_add = [(inner, 1)]
        count = 0
        while to_add:
            print(to_add)
            outer, outer_quantity = to_add.pop()
            reqs = self.must_contain.get(outer, {}).items()
            for bag, inner_quantity in reqs:
                if bag is not None:
                    to_add.append((bag, outer_quantity * inner_quantity))
            count += outer_quantity
        return count - 1  # don't counter bag itself
